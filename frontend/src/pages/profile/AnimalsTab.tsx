...
export default function AnimalsTab({ animals, isLoading, canCreate }: Props) {
  return (
    <div className="space-y-6">
      ...
      {isLoading && (
        <p className="text-center text-gray-500 dark:text-slate-400 py-8">Loading animals...</p>
      )}
          ...
          {animals.map((animal) => (
            ...
            >
              <div className="flex items-start justify-between gap-2 mb-2">
                ...
                <span className={`shrink-0 px-2 py-0.5 rounded-full text-xs font-semibold capitalize ${getStatusColor(animal.status?.name ?? 'available')}`}>
