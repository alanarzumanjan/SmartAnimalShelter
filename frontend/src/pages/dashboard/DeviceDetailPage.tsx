...
type Notice = { level: 'green' | 'orange' | 'red'; title: string; text: string };

function co2Quality(co2: number | null): Quality {
  if (co2 === null) return { title: 'No data', badgeClass: 'bg-slate-100 text-slate-600 dark:bg-slate-800 dark:text-slate-400' };
...
function co2Notice(co2: number | null): Notice | null {
  ...
  if (co2 <= 700) return { level: 'green', title: 'Air looks good ✅', text: 'Great conditions. Keep normal ventilation habits. No action needed.' };
  if (co2 <= 1200) return { level: 'orange', title: 'Air needs freshening 🟧', text: 'Open a window for 5–10 minutes, or increase ventilation. You may feel drowsy soon.' };
  return { level: 'red', title: 'High CO₂ — ventilate now 🟥', text: 'Ventilate immediately (10–15 minutes). Avoid long stays, especially for kids or during sleep.' };
}

function noticeClasses(level: Notice['level']) {
  if (level === 'green') return { box: 'border-emerald-500/25 bg-emerald-500/10', badge: 'bg-emerald-500/15 text-emerald-700 dark:text-emerald-300 border-emerald-500/25', dot: 'bg-emerald-400' };
  if (level === 'orange') return { box: 'border-orange-500/25 bg-orange-500/10', badge: 'bg-orange-500/15 text-orange-700 dark:text-orange-300 border-orange-500/25', dot: 'bg-orange-400' };
  return { box: 'border-red-500/25 bg-red-500/10', badge: 'bg-red-500/15 text-red-700 dark:text-red-300 border-red-500/25', dot: 'bg-red-400' };
}
...
type TooltipPayloadItem = { dataKey?: string | number; name?: string; value?: number | string | null };
type TooltipContentProps = { active?: boolean; payload?: TooltipPayloadItem[]; label?: string | number };
...
const DeviceDetailPage: React.FC = () => {
  ...
  const visibleMeasurements = useMemo(() => measurements.slice(0, showCount), [measurements, showCount]);
  ...
  if (isLoading) {
    return <div className="py-16 text-center text-slate-500 dark:text-slate-400">Loading device...</div>;
  }

  if (!device) {
    return (
      <div className="py-16 text-center">
        <h1 className="text-3xl font-bold text-slate-900 dark:text-white mb-3">Device not found</h1>
    ...
    <div className="space-y-6">
        ...
        <div className="flex items-center gap-2 flex-1 min-w-0">
          {isEditing ? (
            <div className="flex items-center gap-2 flex-1 flex-wrap w-full">
              ...
              />
              <button onClick={handleSaveEdit} className="p-2 rounded-lg bg-emerald-600 text-white hover:bg-emerald-500 transition-colors" title="Save">
                <Check className="w-4 h-4" />
              </button>
              <button onClick={() => setIsEditing(false)} className="p-2 rounded-lg bg-slate-200 dark:bg-slate-700 text-slate-600 dark:text-slate-300 hover:bg-slate-300 dark:hover:bg-slate-600 transition-colors" title="Cancel">
                <X className="w-4 h-4" />
              </button>
            </div>
          ) : (
            <>
              ...
              <p className="text-sm text-slate-500 dark:text-slate-400 ml-2">
                Location: {device.location} · MAC: <span className="font-mono">{device.deviceId}</span>
                {latest ? (
                  <span className="ml-2 text-xs">· Updated: {formatDateTimeForTimeZone(latest.timestamp, timeZone)}</span>
                ) : null}
              </p>
            </>
          )}
      ...
      <div className="flex flex-col gap-2">
        <div className="flex flex-wrap items-center gap-2">
          <span className={`inline-flex items-center gap-2 px-3 py-1.5 rounded-full text-xs font-semibold border ${quality.badgeClass}`}>
            Air quality: <span className="uppercase tracking-wide">{quality.title}</span>
            {latestCo2 != null ? <span className="font-mono opacity-90">· {latestCo2.toFixed(0)} ppm</span> : null}
          </span>
          ...
          <div className={`rounded-2xl border p-4 ${noticeUi.box}`}>
            <div className="flex items-start justify-between gap-3">
              <div>
                ...
                <p className="text-xs text-slate-500 dark:text-slate-400 mt-1">Based on the latest CO₂ reading.</p>
              </div>
              <span className={`inline-flex items-center gap-2 px-3 py-1.5 rounded-full text-xs font-semibold border ${noticeUi.badge}`}>
            ...
            <div className="mt-3">
              ...
              <p className="text-sm text-slate-600 dark:text-slate-300 mt-1">{notice.text}</p>
            </div>
          </div>
        )}
            ...
            {Array.from({ length: 8 }).map((_, i) => {
              ...
              return (
                <div
                  ...
                  style={on ? { boxShadow: '0 0 10px rgba(255,255,255,0.08), 0 0 16px rgba(255,255,255,0.06)' } : undefined}
                />
              );
            })}
          </div>
      ...
      <div className="rounded-2xl bg-slate-100 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 p-4">
        <div className="flex flex-wrap items-start justify-between gap-3">
          ...
          <div className="flex items-center gap-2 text-xs text-slate-500 dark:text-slate-400">
            ...
            <span className="px-2 py-1 rounded-lg border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900">
              {live && presetMs(preset) ? `Live: ON (${LIVE_POLL_MS / 1000}s)` : 'Live: OFF'}
            </span>
          </div>
        </div>
        ...
        <div className="mt-4 flex flex-wrap items-center gap-2">
          ...
          <button
            ...
            title={presetMs(preset) === null ? 'Live works only with preset ranges' : `Auto-refresh every ${LIVE_POLL_MS / 1000}s`}
          >
      ...
      <div className="rounded-2xl bg-slate-100 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 overflow-hidden">
        <div className="flex items-center justify-between px-4 py-3 border-b border-slate-200 dark:border-slate-700">
          <div className="flex items-center gap-2">
            <p className="text-sm font-semibold text-slate-900 dark:text-white">Latest measurements</p>
          </div>
          <p className="text-xs text-slate-500 dark:text-slate-400">
            Showing {Math.min(showCount, measurements.length)} of {measurements.length}
          </p>
        </div>

        {measurements.length === 0 ? (
          <div className="p-12 text-center text-slate-400">No measurements received yet.</div>
        ) : (
          <>
                ...
                <tbody>
                  {visibleMeasurements.map((m) => (
                    <tr key={m.id} className="border-t border-slate-200 dark:border-slate-700">
                      <td className="px-3 py-2 text-slate-500 dark:text-slate-400">{formatDateTimeForTimeZone(m.timestamp, timeZone)}</td>
                      <td className="px-3 py-2 text-right font-semibold text-slate-900 dark:text-white">{m.co2.toFixed(0)}</td>
                      <td className="px-3 py-2 text-right text-slate-500 dark:text-slate-400">{m.temperature.toFixed(1)}</td>
                      <td className="px-3 py-2 text-right text-slate-500 dark:text-slate-400">{m.humidity.toFixed(0)}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
            ...
            <div className="flex flex-wrap gap-2 items-center justify-between px-4 py-3 border-t border-slate-200 dark:border-slate-700">
              ...
              <div className="flex gap-2">
                {showCount > PAGE_SIZE && (
                  <button type="button" onClick={() => setShowCount(PAGE_SIZE)}
                ...
                <button
                  ...
                  onClick={() => setShowCount((v) => Math.min(v + PAGE_SIZE, measurements.length))}
                  className={`px-3 py-1.5 rounded-xl text-xs font-semibold transition-colors ${
                    canShowMore ? 'bg-indigo-600 hover:bg-indigo-500 text-white' : 'bg-slate-200 text-slate-500 dark:bg-slate-700 dark:text-slate-500 cursor-not-allowed'
                  }`}
                >
                  + Show more (20)
                </button>
