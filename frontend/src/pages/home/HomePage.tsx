    ...
    <div className="py-12 space-y-10">
        ...
        <div className="relative text-center">
          ...
          <div className="flex flex-wrap gap-4 justify-center">
            {isAuthenticated ? (
              ...
              <>
                <Link to="/login">
                  <Button variant="primary" className="px-6 py-3">Sign In</Button>
                </Link>
                <Link to="/register">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8 max-w-6xl mx-auto">
        <div className="rounded-[1.75rem] border border-white/70 bg-white/80 p-6 shadow-[0_18px_60px_-30px_rgba(15,23,42,0.22)] backdrop-blur-xl dark:border-white/10 dark:bg-slate-900/70 dark:shadow-[0_28px_80px_-40px_rgba(2,6,23,0.82)]">
          ...
          <h3 className="mb-2 text-lg font-bold text-slate-900 dark:text-white">Animal Records</h3>
          <p className="text-sm leading-6 text-slate-500 dark:text-slate-400">Pet profiles, health history, status tracking, and room for future data imports from rescue and shelter workflows.</p>
        </div>
        <div className="rounded-[1.75rem] border border-white/70 bg-white/80 p-6 shadow-[0_18px_60px_-30px_rgba(15,23,42,0.22)] backdrop-blur-xl dark:border-white/10 dark:bg-slate-900/70 dark:shadow-[0_28px_80px_-40px_rgba(2,6,23,0.82)]">
          <Wifi className="mb-4 h-10 w-10 text-blue-500" />
          <h3 className="mb-2 text-lg font-bold text-slate-900 dark:text-white">IoT Monitoring</h3>
          <p className="text-sm leading-6 text-slate-500 dark:text-slate-400">CO2, temperature, and humidity tracking with alerting logic that can later connect directly to medical and enclosure history.</p>
        </div>
        <div className="rounded-[1.75rem] border border-white/70 bg-white/80 p-6 shadow-[0_18px_60px_-30px_rgba(15,23,42,0.22)] backdrop-blur-xl dark:border-white/10 dark:bg-slate-900/70 dark:shadow-[0_28px_80px_-40px_rgba(2,6,23,0.82)]">
          <MessageSquare className="mb-4 h-10 w-10 text-emerald-500" />
          <h3 className="mb-2 text-lg font-bold text-slate-900 dark:text-white">Chat and Support</h3>
          <p className="text-sm leading-6 text-slate-500 dark:text-slate-400">A shared space for adopters and shelter teams, ready for contextual messaging around requests, visits, and future orders.</p>
        </div>
        <div className="rounded-[1.75rem] border border-white/70 bg-white/80 p-6 shadow-[0_18px_60px_-30px_rgba(15,23,42,0.22)] backdrop-blur-xl dark:border-white/10 dark:bg-slate-900/70 dark:shadow-[0_28px_80px_-40px_rgba(2,6,23,0.82)]">
          <BarChart3 className="mb-4 h-10 w-10 text-amber-500" />
          <h3 className="mb-2 text-lg font-bold text-slate-900 dark:text-white">Equipment Store</h3>
          <p className="text-sm leading-6 text-slate-500 dark:text-slate-400">A preview of shelter-ready devices and accessories with space for future pricing, stock, ordering, and delivery statuses.</p>
        </div>
      </div>
      ...
      <div className="rounded-[2rem] border border-primary-200/70 bg-gradient-to-r from-primary-100 via-white to-cyan-50 p-10 text-center shadow-[0_20px_70px_-30px_rgba(37,99,235,0.3)] dark:border-primary-400/15 dark:from-slate-900 dark:via-slate-900/95 dark:to-cyan-950/40 dark:shadow-[0_30px_90px_-40px_rgba(37,99,235,0.5)]">
        <h2 className="mb-2 text-2xl font-bold text-slate-900 dark:text-white">Build a smarter shelter workflow</h2>
        <p className="mb-5 text-slate-600 dark:text-slate-300">Track care, explore animals, and prepare for connected operations from one platform.</p>
        {isAuthenticated ? (
          <Link to="/dashboard">
