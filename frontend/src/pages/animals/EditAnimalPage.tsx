...
export default function EditAnimalPage() {
  ...
  useEffect(() => {
    async function load() {
      try {
        ...
        let ageValue = '', ageUnit: 'years' | 'months' = 'years';
        if (ageYears != null) {
          if (ageYears < 1) { ageValue = Math.round(ageYears * 12).toString(); ageUnit = 'months'; }
          else { ageValue = ageYears.toString(); }
        }
        ...
        setF({
          ...
          currentMedications: a.currentMedications || a.CurrentMedications || '',
          ...
          housingRequirement: a.housingRequirement || a.HousingRequirement || '',
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    try {
      ...
      if (f.breed !== currentBreedName) {
        try { await api.patch(`/pets/${animalId}/breed`, { breedName: f.breed }); } catch { /* ignore */ }
      }
      ...
      if (imageFile) {
        ...
        try {
          await api.post(`/pets/${animalId}/image`, form, { headers: { 'Content-Type': 'multipart/form-data' } });
  ...
  async function handleDelete() {
    if (!window.confirm('Delete this animal profile? This cannot be undone.')) return;
    setIsDeleting(true);
    try {
  ...
  return (
    <div className="py-8">
      <div className="flex items-center justify-between mb-6">
        ...
        <Button variant="destructive" onClick={handleDelete} isLoading={isDeleting}><Trash2 className="w-4 h-4 mr-2" />Delete</Button>
      </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-10">
            ...
            <div className="flex items-start gap-6">
              <div className="shrink-0">
                ...
                {imagePreview && imagePreview !== currentImageUrl && (
                  <button type="button" onClick={() => { setImageFile(null); setImagePreview(currentImageUrl); }} className="mt-2 flex items-center gap-1 text-sm text-red-600">
          ...
          <Section title="Basic Information">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <Input label="Name" value={f.name} onChange={(e: ChangeEvent<HTMLInputElement>) => setF({ ...f, name: e.target.value })} error={errors.name} />
              <div>
