...
const getRegionLabel = (timeZone: string) => {
  ...
  return (
    timeZone.split("/").slice(0, -1).map(humanizeSegment).join(" / ") ||
    "Global"
  );
};
...
const buildTimeZoneOptions = (): TimeZoneOption[] =>
  getSupportedTimeZones()
    .map((timeZone) => {
      const { label: offsetLabel, minutes } =
        getTimeZoneOffsetDetails(timeZone);
      ...
      return {
        ...
        localTimeLabel,
        searchText:
          `${label} ${country} ${timeZone} ${offsetLabel}`.toLowerCase(),
...
export const formatShortDateTimeForTimeZone = (
  value: Date,
  timeZone = getPreferredTimeZone(),
) =>
...
export const formatShortTimeForTimeZone = (
  value: Date,
  timeZone = getPreferredTimeZone(),
) =>
  new Intl.DateTimeFormat("en-GB", {
